...
/**
 * The purpose of the WspDal is to provide the Data Access Layer for OCDT WSP ECSS-E-TM-10-25
 * Annex C, REST API.
 */
export class WspDal implements Dal {
  ...
  /**
   * Initializes a new instance of the WspDal class.
   */
  constructor() {
    this.client = new WebServiceClient('1.0.0');
  }
  ...
  public write(operationContainer: OperationContainer, files?: File []): Observable<Dto.Thing[]> {
    ...
    if (operationContainer.operations.some(this.isNotSupportedOperation)) {
      throw new Error('The WSP DAL does not support Copy or Move operations');
    }
